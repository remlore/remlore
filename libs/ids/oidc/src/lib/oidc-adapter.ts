import { Oidc, PrismaService } from '@remlore/ids/core/prisma'
import { Adapter, JsonObject } from 'oidc-provider'
...
].reduce((map, name, i) => ({ ...map, [name]: i + 1 }), {} as Record<string, number>)

const prepare = (doc: Oidc) => {
  const isPayloadJson =
    doc.payload && typeof doc.payload === 'object' && !Array.isArray(doc.payload)

  const payload = isPayloadJson ? (doc.payload as JsonObject) : {}
...
export class PrismaAdapter implements Adapter {
  ...
  async find(id: string) {
    const doc = await this.prisma.oidc.findUnique({
      where: { id }
    })
  ...
  async findByUserCode(userCode: string) {
    const doc = await this.prisma.oidc.findFirst({
      where: {
        userCode
      }
    })
  ...
  async revokeByGrantId(grantId: string) {
    await this.prisma.oidc.deleteMany({
      where: {
        grantId
      }
    })
